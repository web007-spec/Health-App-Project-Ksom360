...
import { ChevronLeft, ChevronRight, Minus, Plus, Send, Camera, Flame, Trophy, BarChart3, CheckCircle2, MessageSquare } from "lucide-react";
...
export default function ClientHabitDetail() {
  ...
  // Fetch all completions for insights
  ...
  const circumference = Math.PI * radius; // semicircle-ish (270 degrees)
  ...
  const completedDays = allCompletions ? new Set(allCompletions.map((c: any) => c.completion_date)).size : 0;
      ...
      <div className="min-h-screen bg-muted/30">
        ...
        {activeTab === "habit" && (
          <div className="px-4 py-4">
            ...
            {/* Increment controls */}
            {habit.goal_unit === "cups" || habit.goal_unit === "glasses" ? (
              /* Grid of tappable icons for cups/glasses */
              <div className="bg-background rounded-2xl p-5 shadow-sm">
                <p className="text-sm text-center text-muted-foreground mb-3">
                  Add {habit.goal_unit === "cups" ? "water intake" : habit.goal_unit}
                </p>
                  ...
                  {Array.from({ length: goalValue }).map((_, i) => {
                    ...
                    return (
                      <button
                        key={i}
                        onClick={() => {
                          if (!isFilled) addCompletionMutation.mutate();
                        }}
                        className={`w-12 h-12 mx-auto rounded-xl flex items-center justify-center text-xl transition-all ${
                          isFilled
                            ? "bg-primary/10"
                      ...
                      >
                        {isFilled ? "🥤" : <Plus className="h-4 w-4 text-muted-foreground" />}
                      </button>
                    );
                  })}
                </div>
          ...
          <div className="px-4 py-4 space-y-4">
            ...
            <div className="bg-background rounded-2xl p-4 shadow-sm">
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                ...
                {monthDays.map((day) => {
                  const ds = format(day, "yyyy-MM-dd");
                  const isCompleted = completionDates.has(ds);
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-background rounded-2xl p-4 text-center shadow-sm">
                ...
                <div className="flex items-baseline justify-center gap-0.5 mt-1">
                  <span className="text-2xl font-bold">{dailyAvg}</span>
              ...
              <div className="bg-background rounded-2xl p-4 text-center shadow-sm">
                ...
                <div className="flex items-baseline justify-center gap-0.5 mt-1">
                  <span className="text-2xl font-bold">{daysCompletedPercent}</span>
                ...
                <div className="mt-2 space-y-2">
                  {allCompletions.filter((c: any) => c.notes).map((c: any) => (
                    <div key={c.id} className="text-sm border-l-2 border-primary pl-3 py-1">
                      <p className="text-muted-foreground text-[10px]">{format(parseISO(c.completion_date), "MMM d")}</p>
                      <p>{c.notes}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-sm text-muted-foreground mt-2">There is no note yet</p>
              )}
            </div>
          </div>
        )}
          ...
          <div className="flex flex-col" style={{ height: "calc(100vh - 160px)" }}>
            ...
            <div className="border-t bg-background px-4 py-3 flex items-center gap-2">
              ...
              <Input
                ...
                className="flex-1"
                onKeyDown={(e) => {
                  if (e.key === "Enter" && commentText.trim()) sendCommentMutation.mutate();
                }}
              />
