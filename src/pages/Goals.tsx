...
export default function Goals() {
  ...
  const { data: clients } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`client_id, status, client_profile:profiles!client_id(full_name)`)
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ goalId, status }: { goalId: string; status: string }) => {
      const { data: goal } = await supabase.from("fitness_goals").select("*").eq("id", goalId).single();
    ...
    onSuccess: (data) => {
      ...
      if (data.status === "completed" && data.goal) {
        confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 } });
        toast({ title: "🎉 Goal Completed!", description: `${data.clientName || "Client"} achieved: ${data.goal.title}` });
      } else {
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card><CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div><p className="text-sm font-medium text-muted-foreground">Total Goals</p><h3 className="text-3xl font-bold mt-2">{totalGoals}</h3></div>
              <div className="h-12 w-12 rounded-xl bg-primary/10 flex items-center justify-center"><Target className="h-6 w-6 text-primary" /></div>
            </div>
          </CardContent></Card>
          <Card><CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div><p className="text-sm font-medium text-muted-foreground">Active Goals</p><h3 className="text-3xl font-bold mt-2">{activeGoals.length}</h3></div>
          ...
          <Card><CardContent className="p-6">
            <div className="flex items-start justify-between">
              <div><p className="text-sm font-medium text-muted-foreground">Completion Rate</p><h3 className="text-3xl font-bold mt-2">{completionRate}%</h3></div>
              <div className="h-12 w-12 rounded-xl bg-accent/10 flex items-center justify-center"><Trophy className="h-6 w-6 text-accent-foreground" /></div>
            </div>
          </CardContent></Card>
        </div>
        ...
        {clients && clients.length > 0 && (
          <Card><CardContent className="p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {clients.map(client => (
                <Button key={client.client_id} variant="outline" onClick={() => handleCreateGoal(client.client_id, client.client_profile?.full_name || "Unknown")}>
            ...
            {activeGoals.length > 0 ? (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {activeGoals.map((goal: any) => (
                  <GoalCard key={goal.id} goal={goal} onStatusChange={handleStatusChange} isTrainer showClientName />
                ))}
              </div>
            ) : (
            ...
            {completedGoals.length > 0 ? (
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {completedGoals.map((goal: any) => (
                  <GoalCard key={goal.id} goal={goal} isTrainer showClientName />
                ))}
              </div>
            ) : (
              <Card><CardContent className="py-12 text-center">
